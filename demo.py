...
for n in range(2):
    ...
    w_normalized = _normalize_vector(w)
    # p3 = np.random.choice(p, N, replace=True, p=w_normalized).tolist()
    ...
    print('-' * 60)
    print('# particles: {}'.format(len(set(p))))

    for i in set(p):
        print(i, w_normalized[p.index(i)])
